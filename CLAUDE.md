...
- **Choosing between them.** Measured on this repository; quality first, tokens second.
  ...
  | Is the symbol name unique (first step)                 | **`npm run find:symbol -- <name>`**                       | Answers uniqueness in milliseconds without external binaries, reports exact declarations, warns on client/server drift, and routes to Serena or rg.           |
