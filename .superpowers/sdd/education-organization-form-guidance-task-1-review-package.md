...
Base: untracked pre-existing test file in shared worktree
Head: current working tree after Task 1 RED edits

## Full file snapshot

Path: `client/src/widgets/admin-education-organizations-workspace/ui/education-organizations-cards.test.tsx`
...
describe('education organization create and edit cards', () => {
  ...
  it('renders accessible grouped operator inputs in the create form', () => {
    ...
    expect(
      screen.getByText(/обязательно для обработки ПДн от имени организации/i),
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText(/неполную или неактивную организацию нельзя использовать/i),
    ).toBeInTheDocument();
  });
